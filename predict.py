...
if __name__ == "__main__":
    ...
    with open(args.test_list, mode='r') as csv_file:
        ...
        for row in tqdm(csv_reader):
            prot = row[0]
            #try:
            embed_index = str(prot).replace(" ","\t") 
            #print("this protein is:", "<"+embed_index+">")
            prot = prot.split()[0]
            print("the protein is: ",prot)
            with h5py.File(args.res_embeddings, "r") as f:
                access_data_key = list(f.keys())[0]
                main_group =f[access_data_key] 
                for sub_key in main_group.keys():
                    #print(main_group[sub_key].keys())
                    if embed_index in main_group[sub_key].keys():
                        print("it was found")
                        res_embed = main_group[sub_key][embed_index][:]
                        break
                    else:
                        continue
            print("res embed",res_embed)
